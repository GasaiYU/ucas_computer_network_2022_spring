...
void arpcache_append_packet(iface_info_t *iface, u32 ip4, char *packet, int len)
{
	...
	struct arp_req *new = malloc(sizeof(struct arp_req));
	new->iface = iface;
	new->ip4 = ip4;
	new->sent = time(NULL);
	new->retries = 1;
	init_list_head(&(new->cached_packets));
	list_add_tail(&(recv_pkt->list),&(new->cached_packets));
	list_add_tail(&(new->list),&(arpcache.req_list));
